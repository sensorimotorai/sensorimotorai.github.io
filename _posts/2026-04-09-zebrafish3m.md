...
The specific dataset comes from a preparation where a larval zebrafish is head-fixed in a virtual reality environment ([Mu et al. (2019), from Misha Ahrens' lab](https://www.cell.com/cell/fulltext/S0092-8674(19)30621-X){:target="_blank" rel="noopener"}). A striped ground plane drifts beneath it, simulating a current. When the sensorimotor loop is **closed**, the fish's tail movements control the visual feedback: it swims and sees itself swimming. When the loop is **open**, the visual feedback is disconnected. The fish tries to swim but nothing happens.
...
1. **Active pre-training:** The agent swims freely in a naturalistic environment, learning a forward model of transition dynamics. This model becomes its fixed prior: its "compass needle."

2. **Exploration:** The agent is placed in the experimental protocol (head-fixed, open-loop). It also learns an online model of the new dynamics. The intrinsic reward is the absolute value of the difference between the online model's prediction residual (relative to the prior) and a leaky-integrated running average of that residual.
